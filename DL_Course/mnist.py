...
def plt_md(str, *kwg):  # 精度图,loss图,方差图
    ...
    plt.legend()
    plt.savefig(r'D:\PyCharm\PyLab\DL_Course\Module\{}.png'.format(str))
    plt.show()
...
def test():
    # 加载模型
    nn_net = torch.load(r'D:\PyCharm\PyLab\DL_Course\Module\MNIST.pt')
    # test不用epoch
    nn_net.eval()  # 模型验证
...
def test_man(path):     # path为手写图片的存储路径
    res=[]      # 用来存储预测值
    nn.net=torch.load(r'D:\PyCharm\PyLab\DL_Course\Module\MNIST.pt')       # 加载模型
